...
class Mars(object):
    ...
    def __init__(self, root='/data/datasets/', min_seq_len=0):
        ...
        self.track_train_info_path = osp.join(self.root, 'tiger_MARS_database20191202/train_info.npy')
        self.track_test_info_path = osp.join(self.root, 'tiger_MARS_database20191202/test_info.npy')
        self.query_IDX_path = osp.join(self.root, 'tiger_MARS_database20191202/query_IDX.npy')
        self.root = osp.join(self.root,'tigerData20191202')
        self._check_before_run()

        # prepare meta data
        train_names = self._get_names(self.train_name_path)
        test_names = self._get_names(self.test_name_path)
        track_train = np.load(self.track_train_info_path)#['track_train_info'] # numpy.ndarray (8298, 4)
        track_test = np.load(self.track_test_info_path)#['track_test_info'] # numpy.ndarray (12180, 4)
        query_IDX = np.load(self.query_IDX_path).squeeze()#['query_IDX'].squeeze() # numpy.ndarray (1980,)
